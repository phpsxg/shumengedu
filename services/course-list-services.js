 ...
 * 
 * 
 * 列表 有type_id 为对应分类下的 文章 无 代表所有文章
 */
// 分页函数
function getPageOfMemo(pageNumber) {
    ...
    $.ajax({
        ...
        success : function(response) {
        		...
        		var webTitle = response.data.course_type;
        		if(webTitle){
				$(document).attr("title",webTitle.title);
				$('meta[name="Keywords"]').attr("content",webTitle.keywords);
				$('meta[name="Description"]').attr("content",webTitle.description);
        		}else {
        			$("title").attr("title","");
        		}
//      		console.log(webTitle);
            ...
            if (response.data != null) {
                //匹配标题
                $("#courseTypeTitle").html(response.msg);
                    ...
                    '<div class="panel-body">'+
                        '<div class="row">'+
                            ...
                            '<div class="positon-relative col-lg-8 col-md-8 col-sm-12 col-xs-12">'+
                                ...
                                '</div>'+
                                '<div>'+
                                    '<p class="pt-10">'+item.content+
                                    '</p>'+
                                '</div>'+
                            '</div>'+
                        '</div>'+
                            '<div class="row border-bottom">'+
                                '<div class="col-lg-4 col-md-3 col-sm-12 col-xs-12">时间：'+
                                    item.add_time+
                                '</div>'+
                                ...
                                '</div>'+
                                '<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">作者：'+
                                    item.author_name+
                                '</div>'+
                                '<div class="col-lg-2 col-md-3 col-sm-12 col-xs-12">阅读：'+
                                   +item.pviews+
                                '</div>'+
                            '</div>'+
                        '</div>'+
